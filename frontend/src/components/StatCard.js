...
const StatCard = ({ title, value, icon: Icon, color = 'blue', subtitle, link }) => {
  ...
  if (isAdmin) {
    ...
    return (
      <CardContent>
        <div className={`relative bg-[#1C2540] border border-white/[0.07] rounded-2xl p-5 flex items-center gap-4 ${link ? 'clay-card-hover cursor-pointer' : ''} shadow-clay-admin ${link ? 'hover:shadow-clay-admin-hover' : ''} overflow-hidden ${cfg.glow}`}>
